...
node*sil(node*r,int x)//node* yazildi cunku eger bastaki eleman silinecekse root degistirilmesi gerekli
{
    node*temp;
    node*iter=r;
    ...
    while(iter->next!=NULL && iter->next->x !=x)//iter dedigimiz deger silinecek olan degere gidiyor
    {
        iter=iter->next;

    }
    if(iter->next==NULL)//eger iter null donerse silinecek olan deger dongude yok demektir
    {
        printf("sayi bulunamadi");
        return r ;
    }
    // iterimizi tempe atayip silme islemi
    temp=iter->next;
    iter->next= temp->next;
    free(temp);
    return r;
